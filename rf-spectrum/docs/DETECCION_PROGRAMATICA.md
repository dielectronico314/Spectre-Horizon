...
2. **`scripts/probe.sh`**: Un script envoltorio (wrapper) en Bash diseñado para el sistema host. Su función es ejecutar el script de Python en el contenedor y, críticamente, **silenciar la salida de error estándar (stderr)**. Esto evita que los logs internos de inicialización de C++ contaminen la salida de datos.
